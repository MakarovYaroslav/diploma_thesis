...
from main_functions import get_config, save_user_data_to_db, user_have_enough_comments_count
    ...
    if request.method == 'POST':
        ...
        if social_network == 'reddit':
            if not RedditUser.user_is_exist(username):
                return render_template('main.html', topics=topics, user_error=True, network=social_network)
            ...
            if not TwitterUser.user_is_exist(username):
                return render_template('main.html', topics=topics, user_error=True, network='twitter')
            last_timestamp = get_last_timestamp(TwitterComments, username)
        save_user_data_to_db(social_network, username, last_timestamp)
        response = make_response(redirect(url_for('main.show_results', network=social_network,
                                                  user=username, checked_topics=checked_topics)))
        return response
    else:
        return render_template('main.html', topics=topics, user_error=False, network='reddit')
