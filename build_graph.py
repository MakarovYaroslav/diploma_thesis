...
def draw_graph(network, user, checked_topics):
    ...
    if network == 'reddit':
        with app.app_context():
            comments = RedditComments.query.filter_by(user=user).order_by(RedditComments.timestamp).options(joinedload(RedditComments.result)).all()
    elif network == 'twitter':
        with app.app_context():
            comments = TwitterComments.query.filter_by(user=user).order_by(TwitterComments.timestamp).options(joinedload(TwitterComments.result)).all()

    dates = [datetime.fromtimestamp(comment.timestamp) for comment in comments]
    ocean_attitude = [comment.result.pos * comment.result.politics for comment in comments]
    program_attitude = [comment.result.pos * comment.result.program for comment in comments]
    medicine_attitude = [comment.result.pos * comment.result.medicine for comment in comments]

    ocean_visible = True if 'Politics' in checked_topics else 'legendonly'
    program_visible = True if 'Programming languages' in checked_topics else 'legendonly'
    ...
    program_trace = go.Scatter(x=dates, y=program_attitude,
                               name='Programming languages', visible=program_visible)
    ...
    layout = dict(
        title='Diagram of positive attitude to topics in time with time range slider and selectors',
        xaxis=dict(
            rangeselector=dict(
                buttons=list([
                    dict(count=1,
    ...
    plot_url = py.plot(fig, filename='styling-names', auto_open=False, showLink=False)
    return plot_url
...
def draw_count_graph(network, user):
    ...
    with app.app_context():
        if network == 'reddit':
            comments = pd.read_sql(RedditComments.query.filter_by(user=user).options(joinedload(RedditComments.result)).statement, db.engine)
        elif network == 'twitter':
            comments = pd.read_sql(
                TwitterComments.query.filter_by(user=user).options(joinedload(TwitterComments.result)).statement,
                db.engine)
    ...
    pos_count = pos_comments.groupby(pd.Grouper(key='timestamp', freq=date_delta))['pos'].count()
    neg_count = neg_comments.groupby(pd.Grouper(key='timestamp', freq=date_delta))['neg'].count()
    ...
    plot_url = py.plot(fig, filename='grouped-bar', auto_open=False, showLink=False)
    return plot_url
...
def draw_pie_graph(network, user, checked_topics):
    ...
    if network == 'reddit':
        with app.app_context():
           comments = RedditComments.query.filter_by(user=user).options(joinedload(RedditComments.result)).all()
    elif network == 'twitter':
        with app.app_context():
            comments = TwitterComments.query.filter_by(user=user).options(joinedload(TwitterComments.result)).all()
